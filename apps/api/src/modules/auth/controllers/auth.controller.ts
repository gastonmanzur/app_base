...
export const authController = {
  register: async (req: Request, res: Response): Promise<void> => {
    ...
    res.status(201).json({ success: true, data: { message: 'Registration successful. Check your email to verify account.' } });
  },
  ...
  forgotPassword: async (req: Request, res: Response): Promise<void> => {
    ...
    res.status(200).json({ success: true, data: { message: 'If the account exists, a reset email has been sent.' } });
  },
  ...
  changePassword: async (req: AuthenticatedRequest, res: Response): Promise<void> => {
    ...
    await authService.changePassword(req.auth!.userId, data.currentPassword, data.newPassword);
    res.status(200).json({ success: true, data: { message: 'Password changed successfully. Please login again.' } });
  },
  ...
  adminOnly: async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
    res.status(200).json({ success: true, data: { message: 'Admin resource granted' } });
  }
};
